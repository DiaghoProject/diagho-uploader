...
def main():
    ...
    parser.add_argument('--input_type', type=str, help="Type of the input file ('tsv' or 'json').")
    ...
    parser.add_argument('--target_directory', type=str, help="Path to the final target directory for JSON files for the uploader.")
    ...
    if input_file_type == "tsv":
        input_file_tsv = input_file
        ## Créer le json simple
        file_json_simple = output_directory + "/" + output_prefix + ".simple.json"

        diagho_tsv2json(input_file_tsv, file_json_simple)
    
    else:
        file_json_simple = input_file

    ## TODO #14 Fonction pour valider certaines infos dans le fichier d'input
    ...
    ## Copie les fichiers nécessaire à l'import dans l'uploader
    pattern = os.path.join(output_directory, '*.*family.*')
    fichiers = glob.glob(pattern)
    for fichier in fichiers:
        # Extraire le nom du fichier
        nom_fichier = os.path.basename(fichier)
        # Construire le chemin de destination
        dest_fichier = os.path.join(target_directory, nom_fichier)
        # Copier le fichier
        shutil.copyfile(fichier, dest_fichier)
        print(f"Copié: {fichier} -> {dest_fichier}")


        
    pattern = os.path.join(output_directory, '*.*interpretation.*')
    fichiers = glob.glob(pattern)
    for fichier in fichiers:
        # Extraire le nom du fichier
        nom_fichier = os.path.basename(fichier)
